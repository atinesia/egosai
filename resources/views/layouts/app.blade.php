...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="w-60 bg-slate-900 text-slate-200 flex flex-col">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 text-sm">
            ...
            <a href="{{ route('dashboard.reports') }}"       class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-800 {{ request()->routeIs('dashboard.reports') ? 'bg-slate-800 text-white' : '' }}">📊 Laporan</a>
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.outside="open = false" x-cloak
                    ...
                    <div class="px-4 py-2.5 border-b border-slate-100 flex justify-between items-center">
                        ...
                        <button @click="$store.notif.clearAll()" class="text-[10px] text-slate-400 hover:text-slate-600">Hapus semua</button>
                    </div>
                    <div class="max-h-72 overflow-y-auto divide-y divide-slate-50">
                        <template x-for="n in $store.notif.items" :key="n.id">
                            <a :href="'/dashboard/inbox?conv=' + n.conversation_id"
                               class="flex gap-3 px-4 py-3 hover:bg-slate-50 transition-colors block">
                                ...
                                <div class="min-w-0">
                                    <p class="text-xs font-semibold text-slate-900 truncate" x-text="n.contact_name"></p>
                        ...
                        <div x-show="$store.notif.items.length === 0" class="px-4 py-6 text-center text-xs text-slate-400">
